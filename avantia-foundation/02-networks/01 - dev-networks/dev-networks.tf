################### DEV GKE NETWORKS - US CENTRAL1 A ###################
...
resource "local_file" "export_subnet" {
  ...
  filename = "../../local/dev_gke_subnets_us_central1-a.json"
}

################### DEV GKE NETWORKS - US CENTRAL1 B ###################
...
resource "local_file" "export_subnet_gke_us_ce1_b1" {
  content = jsonencode({
    ...
    subnet_gke_weava_pri_dev_gke_us_ceb_name      = google_compute_subnetwork.subnet_avantia_weava_pri_dev_gke_us_ceb.self_link
    ...
    subnet_gke_weava_pri_dev_gke_us_ceb_name_services_range = google_compute_subnetwork.subnet_avantia_weava_pri_dev_gke_us_ceb.secondary_ip_range[0].range_name
    subnet_gke_weava_pri_dev_gke_us_ceb_name_pod_range = google_compute_subnetwork.subnet_avantia_weava_pri_dev_gke_us_ceb.secondary_ip_range[1].range_name
  ...
  filename = "../../local/dev_gke_subnets_us_central1-b.json"
}

################### DEV GKE NETWORKS - US CENTRAL1 C ###################
...
resource "local_file" "export_subnet_gke_us_ce1_c1" {
  ...
  filename = "../../local/dev_gke_subnets_us_central1-c.json"
}

################# DEV GCE's NETWORK SETTINGS FROM US CENTRAL1 A #################
...
resource "local_file" "export_subnet_gce_cea" {
  ...
  filename = "../../local/dev_gce_subnets_us_central1-a.json"
}

################# DEV GCE's NETWORK SETTINGS FROM US CENTRAL1 B #################
...
################# DEV GCE's NETWORK SETTINGS FROM US CENTRAL1 C #################
...
################# DEV ENDPOINTS AND SVCS's NETWORK SETTINGS FROM US CENTRAL1 A #################
...
resource "local_file" "export_subnet_dev_end_svc_cea" {
  ...
  filename = "../../local/dev_end_svc_subnets_us_central1-a.json"
}

################# DEV ENDPOINTS AND SVCS's NETWORK SETTINGS FROM US CENTRAL1 B #################
...
resource "local_file" "export_subnet_dev_end_svc_ceb" {
  ...
  filename = "../../local/dev_end_svc_subnets_us_central1-b.json"
}
...
resource "local_file" "export_subnet_dev_end_svc_cec" {
  ...
  filename = "../../local/dev_end_svc_subnets_us_central1-c.json"
}